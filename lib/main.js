...
function onConvertSVG() {
    clearOverlay();
    var svgPack = generateSVG(trackList, map.getProjection());
    clearPolylines()
    imageOverlay = new ImageOverlay(svgPack.bounds, svgPack.svg, map);
}
...
function onConvertImage() {
    ...
    image.onload = function() {
        imageOverlay = new ImageOverlay(
            svgPack.bounds, image, map
        );
    };
    image.src = "data:image/svg+xml," + encodeURIComponent( (new XMLSerializer).serializeToString(svgPack.svg));
}
