...
var trackList = [];
var polylineList = [];
var statusTrackCountDiv = document.getElementById("trackCount");
var statusPointCountDiv = document.getElementById("pointCount");
...
function generateNewTrack() {
    var total = parseInt(document.getElementById("pointNumInput").value);
    // Check value
    if (total < 2) {
...
function updateStatusCount() {
    statusTrackCountDiv.innerHTML = trackList.length;
    var count = 0;
    for (var i = 0; i < trackList.length; i++) {
        count += trackList[i].length;
    }
    statusPointCountDiv.innerHTML = count;
}
